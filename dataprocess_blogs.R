...
while (iterate){
  ...
  writedata(dictionary, interatorindex,"dicblog")
  writedata(ngram, interatorindex,"fivegramblog")
...
print("2 - merge de diccionario")

Maxfiles = interatorindex ## 182
dictionary <- readdata(1,"dicblog")

for (i in 2:Maxfiles){
  temp  <- readdata(i,"dicblog")
...
writedata(x=dictionary,file="dictionary_blog.dat")

### 3: Clean dictionary
print("clean diccionario")
...
writedata(x=final_dictionary, file="final_dictionary_blog.dat")
writedata(x=wordstodeleteNgram, file="wordstodeleteNgram_blog.dat")

## 4: limpiar los Ngramas - quito ngramas con freq=1
print("limpiar Ngramas")

for (i in 1:Maxfiles){
  print(i)
  tmp <- readdata(i,"fivegramblog")
  tmp <-tmp[tmp$Freq>1,]
  writedata(tmp,index = i,"finalfivegram_blog")
}

## 5: merge de los ngramas
print("merge Ngramas")

mergedngram <- readdata(1,"finalfivegram_blog")

for (i in 2:Maxfiles){
  print(i)
  temp  <- readdata(i,"finalfivegram_blog")
...
writedata(x=mergedngram,file="blog_merged5gram.dat")
